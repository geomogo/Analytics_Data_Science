...
payment_vars_test = train.groupby('customer_ID').agg({'P_2':['mean', 'median', 'sum', 'count', data_range, iqr, avg_pct_change, correlation], 'P_3':['mean', 'median', 'sum', 'count', data_range, iqr, avg_pct_change, correlation], 'P_4':['mean', 'median', 'sum', data_range, iqr, avg_pct_change, correlation]}).reset_index(drop = False)
...
## Exporting the resulting training and testing data-frames to csv files
