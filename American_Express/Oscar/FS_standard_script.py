...
for i in tqdm(range(0, 10)):
    ...
    RF_auto_feature = RFECV(estimator = RandomForestClassifier(n_estimators = 50, max_depth = 3), step = 40, scoring = amex_function, min_features_to_select = 10, cv = 3, n_jobs = -1).fit(X_train, Y_train)
...
features_to_select.to_csv('Delinquency_Features_to_select_4.csv', index = False)