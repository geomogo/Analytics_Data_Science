...
#               'R_7': 'float16','S_3': 'float16','D_41': 'float16','B_3': 'float16','D_42': 'float16','D_43': 'float16','D_44': 'float16',
#               'B_4': 'float16','D_45': 'float16','B_5': 'float16','R_7': 'float16','D_46': 'float16','D_47': 'float16','D_48': 'float16',
#               'D_49': 'float16','B_6': 'float16','B_7': 'float16','B_8': 'float16','D_50': 'float16','D_51': 'float16','B_9': 'float16',
#               'R_7': 'float16','D_52': 'float16','P_3': 'float16','B_10': 'float16','D_53': 'float16','S_5': 'float16','B_11': 'float16',
#               'S_6': 'float16','D_54': 'float16','R_7': 'float16','S_7': 'float16','B_12': 'float16','S_8': 'float16','D_55': 'float16',
#               'D_56': 'float16','B_13': 'float16','R_7': 'float16','D_58': 'float16','S_9': 'float16','B_14': 'float16','D_59': 'float16',
...
#               'D_68': 'float16','S_12': 'float16','R_7': 'float16','S_13': 'float16','B_21': 'float16','D_69': 'float16','B_22': 'float16',
...
#               'R_70': 'float16','R_71': 'float16','B_27': 'float16','D_81': 'float16','D_82': 'float16','S_17': 'float16','R_72': 'float16',
#               'B_28': 'float16','R_73': 'float16','D_83': 'float16','R_74': 'float16','R_75': 'float16','D_84': 'float16','R_76': 'float16',
#               'B_29': 'float16','B_30': 'float16','S_18': 'float16','D_86': 'float16','D_87': 'float16','R_77': 'float16','R_78': 'float16',
#               'D_88': 'float16','B_31': 'int64','S_19': 'float16','R_79': 'float16','B_32': 'float16','S_20': 'float16','R_70': 'float16',
#               'R_71': 'float16','B_33': 'float16','D_89': 'float16','R_72': 'float16','R_73': 'float16','D_91': 'float16','D_92': 'float16',
#               'D_93': 'float16','D_94': 'float16','R_74': 'float16','R_75': 'float16','D_96': 'float16','S_22': 'float16','S_23': 'float16',
#               'S_24': 'float16','S_25': 'float16','S_26': 'float16','D_102': 'float16','D_103': 'float16','D_104': 'float16','D_105': 'float16',
#               'D_106': 'float16','D_107': 'float16','B_36': 'float16','B_37': 'float16', 'R_76': 'float16','R_77': 'float16','B_38': 'float16',
...
#               'D_131': 'float16','D_132': 'float16','D_133': 'float16','R_78': 'float16','D_134': 'float16','D_135': 'float16','D_136': 'float16',
...
dtype_dict = {'customer_ID': 'object', 'R_7': 'float16'}

## Reading data-files
train = pd.read_csv(file_content_stream_1, dtype = dtype_dict, usecols = ['customer_ID', 'R_7'])
...
def summary_stats(x):
    ...
    d = {}
    d['R_7_mean'] = x['R_7'].mean()
    d['R_7_median'] = x['R_7'].median()
    d['R_7_min'] = x['R_7'].min()
    d['R_7_max'] = x['R_7'].max()
    d['R_7_range'] = np.where(x['R_7'].shape[0] == 1, 0, x['R_7'].max() - x['R_7'].min())
    d['R_7_IQR'] = np.where(x['R_7'].shape[0] == 1, 0,np.percentile(x['R_7'], 75) - np.percentile(x['R_7'], 25))
    d['R_7_std'] = np.where(x['R_7'].shape[0] == 1, 0, np.std(x['R_7'], ddof = 1))
#     d['R_7_negative_count'] = np.sum(x['R_7'] < 0) 
#     d['R_7_positive_count'] = np.sum(x['R_7'] > 0)
    d['R_7_pct_values_above_mean'] = np.where(x['R_7'].shape[0] == 1, 0, np.sum(x['R_7'] > x['R_7'].mean())/x['R_7'].shape[0])
#     d['R_7_avg_pct_change'] = np.where(x['R_7'].shape[0] == 1, 0, pd.Series(x['R_7'].to_list()).pct_change().mean())
    d['R_7_last_value'] = x['R_7'].iloc[-1]
    
    return pd.Series(d, index = ['R_7_mean', 'R_7_median', 'R_7_min', 'R_7_max', 'R_7_range', 'R_7_IQR', 'R_7_std', 'R_7_pct_values_above_mean', 'R_7_last_value'])
...
# data_change = pd.DataFrame(train_deli.groupby(['customer_ID'])['R_7'].apply(lambda x: pd.Series(x.to_list()).pct_change().mean()))
# data_change['customer_ID'] = data_change.index
# data_change = data_change.reset_index(drop = True)
# data_change.columns = ['R_7_change', 'customer_ID']

# ## Computing change from first to last month
# data_change_first_last = pd.DataFrame(train_deli.groupby(['customer_ID'])['R_7'].apply(lambda x: pd.Series(x.iloc[[0, -1]].to_list()).pct_change())).unstack()
# data_change_first_last = data_change_first_last.drop(columns = ('R_7', 0), axis = 1)
...
# data_change_first_last.columns = ['R_7_change_first_last', 'customer_ID']
