...
#     d['D_41_positive_count'] = np.sum(x['D_41'] > 0)
    d['D_41_values_above_mean'] = np.where(x['D_41'].shape[0] == 1, 0, np.sum(x['D_41'] > x['D_41'].mean()))
    d['D_41_avg_pct_change'] = np.where(x['D_41'].shape[0] == 1, 0, pd.Series(x['D_41'].to_list()).pct_change().mean())
    
    return pd.Series(d, index = ['D_41_mean', 'D_41_median', 'D_41_min', 'D_41_max', 'D_41_range', 'D_41_IQR', 'D_41_std', 'D_41_values_above_mean', 'D_41_avg_pct_change'])
