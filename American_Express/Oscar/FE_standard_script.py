...
def summary_stats(x):
    ...
    d = {}
    d['D_115_mean'] = x['D_115'].mean()
    d['D_115_median'] = x['D_115'].median()
    d['D_115_min'] = x['D_115'].min()
    d['D_115_max'] = x['D_115'].max()
    d['D_115_range'] = np.where(x['D_115'].shape[0] == 1, 0, x['D_115'].max() - x['D_115'].min())
    d['D_115_IQR'] = np.where(x['D_115'].shape[0] == 1, 0,np.percentile(x['D_115'], 75) - np.percentile(x['D_115'], 25))
    d['D_115_std'] = np.where(x['D_115'].shape[0] == 1, 0, np.std(x['D_115'], ddof = 1))
#     d['D_115_negative_count'] = np.sum(x['D_115'] < 0) 
#     d['D_115_positive_count'] = np.sum(x['D_115'] > 0)
    d['D_115_pct_values_above_mean'] = np.where(x['D_115'].shape[0] == 1, 0, np.sum(x['D_115'] > x['D_115'].mean())/x['D_115'].shape[0])
    d['D_115_avg_pct_change'] = np.where(x['D_115'].shape[0] == 1, 0, pd.Series(x['D_115'].to_list()).pct_change().mean())
    
    return pd.Series(d, index = ['D_115_mean', 'D_115_median', 'D_115_min', 'D_115_max', 'D_115_range', 'D_115_IQR', 'D_115_std', 'D_115_pct_values_above_mean', 'D_115_avg_pct_change'])
...
# data_change = pd.DataFrame(train_deli.groupby(['customer_ID'])['D_115'].apply(lambda x: pd.Series(x.to_list()).pct_change().mean()))
# data_change['customer_ID'] = data_change.index
# data_change = data_change.reset_index(drop = True)
# data_change.columns = ['D_115_change', 'customer_ID']

# ## Computing change from first to last month
# data_change_first_last = pd.DataFrame(train_deli.groupby(['customer_ID'])['D_115'].apply(lambda x: pd.Series(x.iloc[[0, -1]].to_list()).pct_change())).unstack()
# data_change_first_last = data_change_first_last.drop(columns = ('D_115', 0), axis = 1)
...
# data_change_first_last.columns = ['D_115_change_first_last', 'customer_ID']
