...
#               'R_6': 'float16','S_3': 'float16','D_41': 'float16','B_3': 'float16','D_42': 'float16','D_43': 'float16','D_44': 'float16',
#               'B_4': 'float16','D_45': 'float16','B_5': 'float16','R_6': 'float16','D_46': 'float16','D_47': 'float16','D_48': 'float16',
#               'D_49': 'float16','B_6': 'float16','B_7': 'float16','B_8': 'float16','D_50': 'float16','D_51': 'float16','B_9': 'float16',
#               'R_6': 'float16','D_52': 'float16','P_3': 'float16','B_10': 'float16','D_53': 'float16','S_5': 'float16','B_11': 'float16',
#               'S_6': 'float16','D_54': 'float16','R_6': 'float16','S_7': 'float16','B_12': 'float16','S_8': 'float16','D_55': 'float16',
#               'D_56': 'float16','B_13': 'float16','R_6': 'float16','D_58': 'float16','S_9': 'float16','B_14': 'float16','D_59': 'float16',
...
#               'R_60': 'float16','R_61': 'float16','B_27': 'float16','D_81': 'float16','D_82': 'float16','S_17': 'float16','R_62': 'float16',
#               'B_28': 'float16','R_63': 'float16','D_83': 'float16','R_64': 'float16','R_65': 'float16','D_84': 'float16','R_66': 'float16',
#               'B_29': 'float16','B_30': 'float16','S_18': 'float16','D_86': 'float16','D_87': 'float16','R_67': 'float16','R_68': 'float16',
#               'D_88': 'float16','B_31': 'int64','S_19': 'float16','R_69': 'float16','B_32': 'float16','S_20': 'float16','R_60': 'float16',
#               'R_61': 'float16','B_33': 'float16','D_89': 'float16','R_62': 'float16','R_63': 'float16','D_91': 'float16','D_92': 'float16',
#               'D_93': 'float16','D_94': 'float16','R_64': 'float16','R_65': 'float16','D_96': 'float16','S_22': 'float16','S_23': 'float16',
#               'S_24': 'float16','S_25': 'float16','S_26': 'float16','D_102': 'float16','D_103': 'float16','D_104': 'float16','D_105': 'float16',
#               'D_106': 'float16','D_107': 'float16','B_36': 'float16','B_37': 'float16', 'R_66': 'float16','R_67': 'float16','B_38': 'float16',
...
#               'D_131': 'float16','D_132': 'float16','D_133': 'float16','R_68': 'float16','D_134': 'float16','D_135': 'float16','D_136': 'float16',
...
dtype_dict = {'customer_ID': 'object', 'R_6': 'float16'}

## Reading data-files
train = pd.read_csv(file_content_stream_1, dtype = dtype_dict, usecols = ['customer_ID', 'R_6'])
...
def summary_stats(x):
    ...
    d['R_6_median'] = x['R_6'].median()
    d['R_6_min'] = x['R_6'].min()
    d['R_6_max'] = x['R_6'].max()
    d['R_6_range'] = np.where(x['R_6'].shape[0] == 1, 0, x['R_6'].max() - x['R_6'].min())
    d['R_6_IQR'] = np.where(x['R_6'].shape[0] == 1, 0,np.percentile(x['R_6'], 75) - np.percentile(x['R_6'], 25))
    d['R_6_std'] = np.where(x['R_6'].shape[0] == 1, 0, np.std(x['R_6'], ddof = 1))
#     d['R_6_negative_count'] = np.sum(x['R_6'] < 0) 
#     d['R_6_positive_count'] = np.sum(x['R_6'] > 0)
    d['R_6_pct_values_above_mean'] = np.where(x['R_6'].shape[0] == 1, 0, np.sum(x['R_6'] > x['R_6'].mean())/x['R_6'].shape[0])
#     d['R_6_avg_pct_change'] = np.where(x['R_6'].shape[0] == 1, 0, pd.Series(x['R_6'].to_list()).pct_change().mean())
    d['R_6_last_value'] = x['R_6'].iloc[-1]
    
    return pd.Series(d, index = ['R_6_mean', 'R_6_median', 'R_6_min', 'R_6_max', 'R_6_range', 'R_6_IQR', 'R_6_std', 'R_6_pct_values_above_mean', 'R_6_last_value'])
...
# data_change = pd.DataFrame(train_deli.groupby(['customer_ID'])['R_6'].apply(lambda x: pd.Series(x.to_list()).pct_change().mean()))
# data_change['customer_ID'] = data_change.index
# data_change = data_change.reset_index(drop = True)
# data_change.columns = ['R_6_change', 'customer_ID']

# ## Computing change from first to last month
# data_change_first_last = pd.DataFrame(train_deli.groupby(['customer_ID'])['R_6'].apply(lambda x: pd.Series(x.iloc[[0, -1]].to_list()).pct_change())).unstack()
# data_change_first_last = data_change_first_last.drop(columns = ('R_6', 0), axis = 1)
...
# data_change_first_last.columns = ['R_6_change_first_last', 'customer_ID']
